        ...
        function renderDockMenu($menus) {
            foreach ($menus as $m) {
                ...
                $isActive = url_is($m['url'].'*') 
                    ? 'bg-yellow-100 text-yellow-800 font-semibold rounded' 
                    : 'text-black hover:bg-yellow-50 hover:text-yellow-800';
                if ($hasChildren) {
                  echo '<div class="relative" x-data="{ showServices: false }">
                        ...
                        <div x-show="showServices" @click.outside="showServices = false" x-cloak x-transition
                            class=" absolute bg-white w-80 text-center shadow-lg rounded-lg z-50 border border-gray-100">';
                    renderDockMenu($m['children']);
                        echo '
                        </div>
                    </div>';
                } else {
                  echo '<a href="'.base_url($m['url']).'"class="text-gray-700 hover:text-[#8c9464] transition-colors duration-200 select-none px-2 py-1 rounded hover:bg-gray-100 flex items-center gap-1 font-bold '.$isActive.' transition-colors duration-200 select-none px-2 py-1 rounded hover:bg-gray-100">
                          '.esc($m['name']).'
                        </a>
                        ';
                }
