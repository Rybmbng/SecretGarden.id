...
<div class="container mx-auto px-6 py-6 font-sans text-gray-800">
                    ...
                    function renderMenu($menus, $level=0){
                        foreach($menus as $m){
                            ...
                            echo '<td class="p-3 space-x-3">
                                    ...
                                    <a href="'. base_url('admin/menu/delete/'.$m['id']) .'" class="text-red-500 hover:underline">Delete</a>
                                  </td>';
                            echo '</tr>';
                            if(!empty($m['children'])){
    ...
    <div class="bg-white shadow-lg rounded-3xl p-6 mb-10 border border-gray-100">
        ...
        <form action="<?= base_url('admin/menu/setRoleAccess') ?>" method="post" id="roleAccessForm">
    ...
    <div id="editModal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center hidden z-50">
        <div class="bg-white rounded-3xl shadow-2xl w-11/12 md:w-1/2 p-6 relative">
            ...
            <form id="editForm" action="<?= base_url('admin/menu/update') ?>" method="post" class="space-y-3">
...
function initIconPicker(pickerId, inputId, searchId){
    ...
    faIcons.forEach(icon=>{
        ...
        i.className = 'fas ' + icon + ' cursor-pointer p-2 border rounded-2xl hover:bg-blue-100 transition';
...
document.getElementById('editForm').addEventListener('submit', async function(e){
    ...
    const res = await fetch(`<?= base_url('admin/menu/update') ?>/${id}`, {
        method: 'POST',
        body: formData
    });
